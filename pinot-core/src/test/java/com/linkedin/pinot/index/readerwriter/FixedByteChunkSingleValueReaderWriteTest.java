...
public class FixedByteChunkSingleValueReaderWriteTest {
  ...
  public void testWithoutCompression()
      throws Exception {
    ChunkCompressorFactory.CompressionType compressionType = ChunkCompressorFactory.CompressionType.NONE;
    testInt(compressionType);
    testLong(compressionType);
    testFloat(compressionType);
    testDouble(compressionType);
  ...
  public void testInt(ChunkCompressorFactory.CompressionType compressionType)
    ...
    for (int i = 0; i < NUM_VALUES; i++) {
      ...
      if (compressionType.equals(ChunkCompressorFactory.CompressionType.NONE)) {
  ...
  /**
   * This test ensures that the reader can read in an data file from previous version.
   * @throws IOException
   */
  @Test
